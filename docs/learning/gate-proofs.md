...
## A test that skips because an optional dependency is absent must run in the job that has it, or it runs nowhere

- **Gate:** `test/test_rl_coverage_contract.py` (4 tests) — run by `python -m unittest`, so it runs in CI's `build` job where the extras are absent, and again in `rl-smoke` where they are present. Written RED against a defect that was already on `main`.
- **What it is for:** `build` installs `requirements-locked.txt` (8 distributions), `rl-smoke` installs `requirements-rl-locked.txt` (39 more) and runs a NAMED module list. A module guarded to skip in the first and missing from the second is tested nowhere, and both jobs stay green.
- **Mutation 0 (the real one, no edit needed):** on 2026-09-02 `test_training_readout_contract.py` was given an RL guard and never added to the `rl-smoke` list, turning 7 errors into `OK (skipped=12)` with those 12 tests losing all remote coverage. The gate was written before that was fixed. **Red:** `these modules skip when the RL extras are absent and are not in the rl-smoke module list, so they run nowhere: test_training_readout_contract`. Green once the module was added to the list.
- **Mutation 1:** `rl-smoke` given `if: false`. **Red:** `the rl-smoke job carries 'if:' ahead of its module list; a module named in a step that may not run is not coverage, and the other checks here cannot see that`. Without this check the module list is still present and every other assertion passes.
- **Mutation 2:** `rl-smoke` switched to install `requirements-locked.txt`. **Red:** `the job that runs the RL module list does not install the RL lockfile before it, so every module named there would skip rather than run`. This is the nastiest variant — the list is intact and every module in it would silently skip.
- **Mutation 3:** `test.test_training_readout_contract` deleted from the list. **Red:** names that module.
- **Mutation 4:** a guard package renamed to `sb3_contribb`. **Red:** `the RL extras are installed here, but these guard names do not resolve: ['sb3_contribb']`. This is the case neither other instrument can see: a permanently-false guard skips in **both** jobs and is reported as `OK (skipped=N)` twice.
- **Green after revert:** yes, 4/4, and the workflow diff back to the single intended line.
- **What the review changed, and this is the important part.** The first version detected guards with three fixed marker strings. It flagged **itself**, because a file listing the markers contains them; tightening the patterns to include their surrounding syntax did not help, since a pattern list necessarily contains its own patterns. It is excluded by name. Worse, a reviewer proved the marker list already missed a spelling introduced **in the same diff** — `self.skipTest("Stable-Baselines3 is optional...")` — so the module was detected only by coincidence, through a second unrelated marker elsewhere in the file. Detection is now "skips AND names an RL package", which covers decorators, bare `SkipTest`, and annotated constants, and takes the guarded set from 13 to 14.
- **Bound, stated in the gate's own header:** it reads what a file SAYS, not what it does. A module that skips for an RL reason without naming one of the five packages is invisible. It reads workflow text, not the job graph, so it cannot see a workflow disabled at the repository level, a runner that never provisions, or a depleted Actions allowance — under which both jobs are dark and every check here is green by absence.

## The local full-suite gate could not see the whole defect class, because the machine that runs it has the dependencies CI does not

- **Gate:** `python scripts/run_without_rl_extras.py -m unittest`, named in AGENTS.md Gates for any change under `test/`, `scripts/` or `src/`.
- **Mutation (the real one):** the working tree as it stood at `d30a7f4`. **Red:** `Ran 1721 tests ... FAILED (errors=11, skipped=28)`, exit 1, and the 11 test ids match CI run 33777420299's 11 exactly. The same tree under plain `python -m unittest` is **green**, which is the whole point: `main`'s `build` job had then been red for 112 consecutive runs, the last green being `d6b5e39` on 2026-08-14.
- **Green after the guards:** `Ran 1721 ... OK (skipped=37)`, exit 0.
- **Independently calibrated, which is what makes it trustworthy.** A reviewer built a real venv from `requirements-locked.txt` rather than believing the harness's account of itself, and ran the suite in it: `Ran 1721, OK (skipped=37)` — identical to the harness on a machine that has the extras. Two different mechanisms, same number.
- **What the review changed.** Three defects, each found by running rather than reading. (1) `importlib.metadata` reported **0 of 149** distributions instead of 144, because `_discover_resolvers()` drops any meta-path entry without `find_distributions` — so *every* package looked uninstalled, including numpy and pygame-ce, and `src/rl/provenance.py` would have recorded every version as null while appearing to pass. (2) The script branch dropped the script's own directory from `sys.path`, so `scripts/search_dataset.py` failed with `No module named 'search_policy'` — the single worst signature for a tool whose entire job is to make `ModuleNotFoundError` mean "a missing RL package". (3) The blocklist was five hand-written roots out of **39 RL-only distributions**: `setuptools`, `packaging`, `networkx`, `sympy` and `jinja2` were provably importable under the harness and provably absent in `build`, and nothing would have noticed the next `uv pip compile` widening the hole. `BLOCKED` is now derived from the two lockfiles on every run, mapped to import roots through `importlib.metadata` because a distribution name is not an import root (pillow ships PIL, protobuf ships google, setuptools ships pkg_resources), and it refuses to run if the derivation does not produce the five roots that matter.
- **Bound, stated in the harness's own header:** this interpreter only — a child started with `sys.executable` sees the real packages, and 14 test modules spawn one, so a test that shells out is outside this gate. Finders appended to `sys.meta_path` after startup are unwrapped. It models absence only as `ModuleNotFoundError`, not as a broken install or an ABI mismatch. And it can only hide what is installed locally, since the distribution-to-root mapping comes from this machine.
