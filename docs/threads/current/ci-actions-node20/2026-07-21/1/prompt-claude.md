...
REVIEWERS MUST READ THE LIVE CODE. Read the actual `.github/workflows/test.yml` in the working tree (it already contains the post-change v7 pins) with the Read tool, and grep the repo for any other reference to these action versions (README.md, ARCHITECTURE.md, PROGRESS.md, docs/, thread evidence). Verify every claim against the live files; do not review from the diff alone.
