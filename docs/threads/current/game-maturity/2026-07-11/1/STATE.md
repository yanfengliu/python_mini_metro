...
Current substep: GM-09a2 - versioned task-descriptor identity (implementation-ci-green; evidence-only `[GM-09a2:B]` active)

Current status: GM-08 is complete; GM-09a (Classic map abstraction) is remotely finalized (Commit A `89ffe3b` run 30024082166, evidence Commit B `0f733a8` [run 30024577509](https://github.com/yanfengliu/python_mini_metro/actions/runs/30024577509) `build` `89265634941` / `rl-smoke` `89265634902`). GM-09a2 Commit A `289a732` passed exact [run 30058894530](https://github.com/yanfengliu/python_mini_metro/actions/runs/30058894530) (`build` job `89376275482`, `rl-smoke` job `89376275539`); evidence-only Commit B `[GM-09a2:B]` is active. GM-09a2 makes the RL task descriptor map-versioned under strict legacy-byte-compat (map-absent stays `c2ef342f`; manifest v3; CLI `--map` with resume-inherit; committed legacy fixture). GM-09 opening is complete; GM-09b (terrain/regions + the first river map) is next.

Durability transaction: GM-07, GM-08, and GM-09a are remotely finalized; GM-09a2 Commit A `289a732` is exact-head remote green, and evidence-only `[GM-09a2:B]` is the active transaction.

Last remotely finalized work unit: GM-09a:B at `0f733a8` (exact [run 30024577509](https://github.com/yanfengliu/python_mini_metro/actions/runs/30024577509) succeeded); GM-09a2 Commit A `289a732` is exact-head remote green pending its evidence Commit B.

Expected remote implementation baseline: `289a732`, whose exact [run 30058894530](https://github.com/yanfengliu/python_mini_metro/actions/runs/30058894530) passed `build` job `89376275482` and `rl-smoke` job `89376275539`.

Current transaction marker: `[GM-09a2:B]`

## Resume here

1. Preserve remotely finalized GM-06/GM-07/GM-08/GM-09a history and the exact-green GM-09a2 Commit A `289a732`, the pre-existing `.agents/` tree, unrelated ignored `output/`, the retained isolated pin, and the live `../civ-engine` sibling.
2. Self-review, stage, commit, and push only the evidence documents in `[GM-09a2:B]` without touching production or unrelated state; wait for Commit B's exact `build` and `rl-smoke` jobs.
3. Then open **GM-09b (terrain/station regions + the first river map)**, reconciling GM-09a2:B. This brings the SECOND map, so the deferred save-schema map field (D-026/Codex-7 fail-closed guard) and the high-score `mapDefinitionVersion` (D-033) must land here (or the guard must stay fail-closed): a save/high-score v2 that synthesizes `classic` for older records, plus terrain/station regions, a river obstacle, and a `MapDefinition` for the river map that `maps.resolve_map`/`map_by_id` expose. Run BOTH review lanes (persistence + new-map surface).
...
| GM-09 | in progress | `89ffe3b` / `0f733a8` / `289a732` | `[GM-09a2:B]` active | Maps, rivers/tunnels — GM-09a (map abstraction) finalized; GM-09a2 (versioned task identity) Commit A `289a732` run 30058894530 succeeded, evidence Commit B active; opening complete, GM-09b (first river map) next |
...
| GM-09a2 | implementation-ci-green | `289a732` / run `30058894530` success | `[GM-09a2:B]` active | Versioned task-descriptor identity (D-033), delivered exact-head remote green (`build` `89376275482`, `rl-smoke` `89376275539`): map-absent descriptor byte-identical (legacy `c2ef342f` preserved; classic-bound `efec72da`), manifest v3 (v1/v2 untouched), `task_spec_from_manifest`/thunk/CLI `--map` with resume-inherit, committed sanitized legacy fixture. Dual review INVERTED — harness CLEAN, external Codex NOT CLEAN (a MAJOR re-dispositioned to the read path + minors, all folded). Evidence finalization remains |
