...
Current substep: GM-09c - crossing/tunnel budget mechanic (implementation-ci-green; evidence-only `[GM-09c:B]` active)

Current status: GM-08 and GM-09a/a2/b are remotely finalized. GM-09c Commit A `7ea087a` passed exact [run 30069945502](https://github.com/yanfengliu/python_mini_metro/actions/runs/30069945502) (`build` job `89408534202`, `rl-smoke` job `89408534263`); evidence-only Commit B `[GM-09c:B]` is active. GM-09c makes the river a real obstacle via a finite per-map tunnel budget (RIVER=3, CLASSIC=None/unbounded): a new dependency-light `src/crossings.py` owns the Liang-Barsky crossing geometry plus the shared `within_tunnel_budget` gate; `Mediator` exposes `num_tunnels`/`consumed_tunnels`/`available_tunnels` as DERIVED properties (free refund, zero snapshot state); the gates count the REAL resolved draft at `end_path_on_station` + the `finish_path_creation` commit boundary; the observation gains a `tunnels` block as a SIBLING of `fleet`; a tunnel-portal marker renders. CLASSIC is byte-identical (gates short-circuit on the None budget; the shared abort/button methods are unchanged). Three dual review rounds converged; the transient-blip and ghost-button on abort are pre-existing HEAD behaviors, deferred to a scoped follow-up. The save-schema/high-score map fields still defer to GM-09f; GM-09d (second map) is next.

Durability transaction: GM-07, GM-08, and GM-09a/a2/b are remotely finalized; GM-09c Commit A `7ea087a` is exact-head remote green, and evidence-only `[GM-09c:B]` is the active transaction.

Last remotely finalized work unit: GM-09b:B at `c4cba8f` (exact [run 30062727652](https://github.com/yanfengliu/python_mini_metro/actions/runs/30062727652) succeeded); GM-09c Commit A `7ea087a` is exact-head remote green pending its evidence Commit B.

Expected remote implementation baseline: `7ea087a`, whose exact [run 30069945502](https://github.com/yanfengliu/python_mini_metro/actions/runs/30069945502) passed `build` job `89408534202` and `rl-smoke` job `89408534263`.

Current transaction marker: `[GM-09c:B]`

## Resume here

1. Preserve remotely finalized GM-06/GM-07/GM-08/GM-09a/a2/b history and the exact-green GM-09c Commit A `7ea087a`, the pre-existing `.agents/` tree, unrelated ignored `output/`, the retained isolated pin, and the live `../civ-engine` sibling.
2. Self-review, stage, commit, and push only the evidence documents in `[GM-09c:B]` (STATE/EVIDENCE) without touching production or unrelated state; wait for Commit B's exact `build` and `rl-smoke` jobs.
3. Then open **GM-09d (second alternate map)**, reconciling GM-09c:B — a second geographic layout reusing the now-proven map layer (`MapDefinition` + regions/rivers/`tunnel_budget`), verifying CLASSIC byte-identity holds and the new map is exercisable end-to-end. Run BOTH review lanes; the external Codex lane has proven the more reliable bug-catcher across GM-09c's three rounds (it caught the commit-boundary bypass, the cached-budget fail-open, and the byte-identity regressions the harness missed) — run it and treat its byte-identity probes as load-bearing. A scoped follow-up (`task_384488d0`) tracks the pre-existing draft-abort snap-blip and ghost-button leaks.
...
| GM-09 | in progress | `89ffe3b` / `0f733a8` / `289a732` / `6ed1d6c` / `44dfecc` / `c4cba8f` / `7ea087a` | `[GM-09c:B]` active | Maps, rivers/tunnels — GM-09a/a2/b remotely finalized; GM-09c (crossing/tunnel budget) Commit A `7ea087a` run 30069945502 succeeded, evidence Commit B active; GM-09d (second map) next |
...
| GM-09c | implementation-ci-green | `7ea087a` / run `30069945502` success | `[GM-09c:B]` active | Crossing/tunnel budget (D-035), delivered exact-head remote green (`build` `89408534202`, `rl-smoke` `89408534263`): dependency-light `src/crossings.py` (Liang-Barsky geometry + shared `within_tunnel_budget` gate), DERIVED `num_tunnels`/`consumed`/`available` (free refund), gates counting the REAL draft at end/finish, `tunnels` observation sibling, tunnel-portal marker; CLASSIC byte-identical. THREE dual review rounds converged (Codex caught the commit-boundary bypass, cached-budget fail-open, and byte-identity regressions the harness missed); the round-2 clean-abort was REVERTED because it broke CLASSIC bytes — the transient-blip/ghost-button on abort are pre-existing HEAD behaviors deferred to `task_384488d0`. Evidence finalization remains |
