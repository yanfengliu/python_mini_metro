...
Current increment: GM-09 - Maps and geographic constraints

Current substep: GM-09f - save-schema v2 map field (implementation-ci-green; evidence-only `[GM-09f:B]` active)

Current status: GM-08 and GM-09a/a2/b/c/d/e are remotely finalized. GM-09f Commit A `cc4d63f` passed exact [run 30079552949](https://github.com/yanfengliu/python_mini_metro/actions/runs/30079552949) (`build` job `89437781481`, `rl-smoke` job `89437781532`); evidence-only Commit B `[GM-09f:B]` is active, reconciling GM-09e:B (`fd49097`, [run 30074995651](https://github.com/yanfengliu/python_mini_metro/actions/runs/30074995651) succeeded). GM-09f is the SAVE-SCHEMA v2 map field (D-038) — the first of a plan-review-driven split (save-schema → high-score identity → in-game menu, the menu LAST so it can never feed an alternate map to the still-classic-hardcoded score recorder). A v2 save records the map IDENTITY (`mapId`/`mapDefinitionVersion`) and rebuilds terrain from the registry on load; `serialize_game` is fail-closed on TWO axes (STRUCTURAL `map_definition == resolve_map(id, version)` + STATE-legal stations-on-land/within-budget, on serialize AND load); the byte-frozen `save-v1.json` is unchanged and still loads as Classic, with the deterministic v1→v2 header-only upgrade pinned by the new frozen `save-v2-classic.json` (15485 bytes, SHA `60f2bc16…`). Dual plan review (both REVISE → structural guard + state-legality) and dual impl review (harness SHIP + Codex FIX-FIRST → folded a latent serialize fail-open + the convergent ASCII/no-whitespace `mapId` mirror gap, with regressions + a forged-over-budget LOAD test). GM-09f2 (high-score `mapDefinitionVersion`) is next, then GM-09f3 (in-game menu).

Durability transaction: GM-07, GM-08, and GM-09a/a2/b/c/d/e are remotely finalized; GM-09f Commit A `cc4d63f` is exact-head remote green, and evidence-only `[GM-09f:B]` is the active transaction.

Last remotely finalized work unit: GM-09e:B at `fd49097` (exact [run 30074995651](https://github.com/yanfengliu/python_mini_metro/actions/runs/30074995651) succeeded); GM-09f Commit A `cc4d63f` is exact-head remote green pending its evidence Commit B.

Expected remote implementation baseline: `cc4d63f`, whose exact [run 30079552949](https://github.com/yanfengliu/python_mini_metro/actions/runs/30079552949) passed `build` job `89437781481` and `rl-smoke` job `89437781532`.

Current transaction marker: `[GM-09f:B]`

## Resume here

1. Preserve remotely finalized GM-06/GM-07/GM-08/GM-09a/a2/b/c/d/e history and the exact-green GM-09f Commit A `cc4d63f`, the pre-existing `.agents/` tree, unrelated ignored `output/`, the retained isolated pin, and the live `../civ-engine` sibling.
2. Self-review, stage, commit, and push only the evidence documents in `[GM-09f:B]` (STATE/EVIDENCE) without touching production or unrelated state; wait for Commit B's exact `build` and `rl-smoke` jobs.
3. Then open **GM-09f2 (high-score `mapDefinitionVersion`)**, reconciling GM-09f:B — the save-schema map field has landed, so the leaderboard must now record which map a score was set on. `main.record_highscore` (`src/main.py`) is still classic-hardcoded; thread the live `map_definition` (id + version) into the recorder and the `highscores` schema so a non-Classic run's score is keyed by map identity, with a migration for the existing schema-v1 leaderboard (an unkeyed board synthesizes `classic`, mirroring the save v1→v2 upgrade). This is HIGH-RISK (persistence migration + a public seam) → escalate to multi-cli-review, run BOTH lanes. GM-09f3 (in-game map menu) lands LAST so it can never expose an alternate map to a still-classic-hardcoded recorder. A scoped follow-up (`task_384488d0`) tracks the pre-existing draft-abort snap-blip and ghost-button leaks.
...
| GM-09 | in progress | `…` / `7ea087a` / `f4b7a49` / `7a522d0` / `5ce3f91` / `a0e87d2` / `fd49097` / `cc4d63f` | `[GM-09f:B]` active | Maps, rivers/tunnels — GM-09a/a2/b/c/d/e remotely finalized; GM-09f (save-schema v2 map field, D-038) Commit A `cc4d63f` run 30079552949 succeeded, evidence Commit B active; GM-09f2 (high-score map identity) + GM-09f3 (in-game menu) next |
...
| GM-09f | implementation-ci-green | `cc4d63f` / run `30079552949` success | `[GM-09f:B]` active | Save-schema v2 map field (D-038), delivered exact-head remote green (`build` `89437781481`, `rl-smoke` `89437781532`): additive `mapId`/`mapDefinitionVersion`, two-phase `validate_save`, fail-closed STRUCTURAL + STATE-legal serialize/load guard, `classic@1` synthesis for v1, byte-frozen `save-v1.json` unchanged + new frozen `save-v2-classic.json` (deterministic v1→v2 upgrade, 15485 bytes, SHA `60f2bc16…`). Dual plan review (both REVISE) + dual impl review (harness SHIP + Codex FIX-FIRST → folded a latent `or CLASSIC` fail-open + the ASCII/no-whitespace `mapId` mirror gap, with regressions + a forged-over-budget LOAD test). First of the GM-09f split; evidence finalization remains |
| GM-09f2 | pending | - | - | High-score `mapDefinitionVersion` — leaderboard keyed by map identity; thread the live `map_definition` into the classic-hardcoded `record_highscore` + a schema migration |
| GM-09f3 | pending | - | - | In-game map menu (classic/river/delta/lake) — LAST, so it can never feed an alternate map to a still-classic-hardcoded recorder |
