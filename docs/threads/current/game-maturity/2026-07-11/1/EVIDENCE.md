...
- Cleanup limitations: ACL-blocked descendants require retaining the old ignored `output/gm04a-precommit-cache`, `output/gm04b-a3-precommit-cache`, `output/gm04b-final-precommit-cache`, and `output/gm04b-precommit-cache-final2` roots. Exact task cache `C:\tmp\python-mini-metro-gm04b-precommit-cache` remains intentionally retained only through GM-04c changed-path hooks; none of these caches is tracked transaction content.
- Delivery boundary: local proof is complete, two independent payload reviews converged `CLEAN` after all findings were corrected, and changed-path hooks plus exact 18-path staging, cached diff, credential, dependency-surface, and exclusion gates pass with no unstaged tracked drift. `[GM-04c:A]` still awaits commit, push, and exact remote CI; no GM-04c commit or remote result is claimed.
