...
At ten frames, the current CNN has 1,507,744 trainable parameters at the `fast` profile and 3,735,968 at `fidelity`. It was 377,248 and profile-independent until 2026-08-13, when the encoder's `AdaptiveAvgPool2d((3, 5))` head was removed: it collapsed every profile to the same 960 values, so each cell averaged 64 x 60 px of a 320 x 180 frame and added resolution never reached the policy. The projection is now sized from the observation space, which is why the count varies by profile. One 256-input/256-hidden PyTorch LSTM has 526,336 parameters; separate actor and critic LSTMs have 1,052,672. Adding the current 308-logit action head and scalar value head makes the CNN/recurrent/head core 1,450,005 parameters. The two live 256-to-64-to-64 policy MLPs bring the exact profiled policy total to 1,491,221. The model is therefore modest; observation and rollout tensors, not weights, are the first memory concern.
