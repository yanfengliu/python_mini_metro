...
def _font(name: str | None, size: int) -> pygame.font.Font:
    """Return a cached deterministic font, mirroring ``LazyRenderResources``."""

    del name  # Text uses pygame's bundled font for portable headless rendering.
    if not pygame.font.get_init():
        pygame.font.init()
        _fonts.clear()
    font = _fonts.get(int(size))
    if font is None:
...
def title_layout(width: int, height: int) -> dict[str, pygame.Rect]:
    ...
    return _stacked_buttons(
        width, ("new_game", "exit"), height // 2 - game_over_button_height // 2
    )
...
def draw_title_screen(surface: pygame.Surface) -> None:
    """Paint deterministic title chrome onto the given surface only."""

    width, height = surface.get_size()
    layout = title_layout(width, height)
    _draw_heading(surface, width, layout["new_game"].top - _HEADING_GAP, _TITLE_HEADING)
    for key, label in (("new_game", "New Game"), ("exit", "Exit")):
