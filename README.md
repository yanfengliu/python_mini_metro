...
* Click the **Map** button on the title screen to cycle the map a New Game will build — `Classic` (open), `River` (one channel), `Delta` (two channels), or `Lake` (a central lake) — each with its own terrain and river-crossing tunnel budget. New Game (and ENTER) builds the selected map; Restart (in-game or at game over) replays the map you were playing, not the picker; and Continue loads whichever map its save was on. The Tutorial always uses Classic.
...
* Every week of play the game pauses at a week boundary and shows a modal with that week's upgrade offers (a seeded pick of New Line / +1 Locomotive / +1 Carriage / +1 Tunnel — tunnel offers appear only on maps with a tunnel budget), one clickable button each; click one to choose it and resume (the sim carries on with no time backlog). The offers are deterministic and reproduce exactly on Continue. Choosing New Line unlocks your next metro line for free (up to the map's line limit); +1 Locomotive and +1 Carriage grow your fleet by one (an extra train or carriage to assign); and +1 Tunnel adds one to the map's river-crossing budget so you can route one more line across the water. Every applied upgrade persists exactly across Continue, and closing the window mid-offer saves the pending boundary so Continue re-enters the modal with the same offers (you don't have to choose before saving). This is an interactive-play feature — headless, RL, and frame-limited runs never pause for a week or generate offers.
...
document = save_game.serialize_game(env.mediator)   # strict schema-v4 dict, never mutates the game
...
- Save documents are versioned strict JSON (`save_schema.SAVE_SCHEMA_VERSION == 4`, supported `{1, 2, 3, 4}`, `stateContract "mini-metro-save-v1"`, `rulesVersion "rules-v1"`). v4 adds an additive `pendingOffers` key (a held week boundary's shown offer kinds; GM-10i) and permits the `"week"` pause reason; v1/v2/v3 saves still load unchanged. `save_schema.validate_save(document)` rejects unknown/missing keys, wrong scalar types (including bool-as-int), forward versions, malformed or out-of-domain RNG state, ID-grammar violations, path or metro references to locked stations, inconsistent bound-service records, and duplicate or dangling entity references; `load_game` additionally rejects duplicate JSON object keys at every level. Every rejection raises `ValueError`.
...
- Save schema v4 (GM-10i, D-047) persists a PENDING week-boundary offer so a mid-offer save Continues back INTO the modal re-presenting the same offers. A v4 document adds one additive top-level key `pendingOffers` (the ordered kinds of the shown offers) and permits the `"week"` pause reason; `SUPPORTED_SAVE_SCHEMA_VERSIONS = {1, 2, 3, 4}` and new saves are v4. The offers are STORED, not re-derived on load — the derivation inputs (`WEEK_LENGTH_STEPS`/`OFFERS_PER_WEEK`/the offer pool) are provisional GM-11-tunable balance defaults, so a re-derive would diverge across a rules change, and the loader deliberately does not couple to them. A v1/v2/v3 document (no `pendingOffers`, no `"week"`) still loads unchanged, so `save-v1.json`/`save-v2-classic.json`/`save-v3-classic.json` stay byte-frozen; re-saving any of them produces the frozen `scripts/fixtures/save-v4-classic.json` (its byte length + SHA are pinned), and `save-v4-river-pending.json` pins a held-boundary save. A save with a `"week"` boundary on a finished game, or a TUNNEL offer on an unbounded map, is rejected fail-closed; `stateContract`/`rulesVersion` stay unchanged.
...
A fresh map-free artifact uses training-manifest v2, which binds a canonical history descriptor and separate `historyFingerprint` without changing the single-frame task fingerprint; a fresh map-bound run (`--map`) uses v3, a v2 superset that additionally records `mapId`/`mapDefinitionVersion` (see the RL `--map` note below). Genuine manifest-v1 artifacts derive the historical contiguous offsets `[frameStack-1, ..., 0]` in memory and retain their exact v1 serialized shape. Fresh training, resume, and evaluation now build the bounded descriptor-driven vector ring from that exact identity. A mismatched explicit history is rejected before model bytes are opened, including equal-channel layouts whose temporal meaning differs; a different channel count is also rejected by SB3 before learning or evaluation.
...
`--map` binds training to a named map — `classic` (the default game), `river` (a central river splitting the map into two banks), `delta` (two rivers splitting the map into three banks; a line spanning both channels uses two tunnels), or `lake` (a single bounded central lake that a line can tunnel through OR detour around). On any water map, stations spawn only on the land and lines cross the water through a finite tunnel budget surfaced in the `tunnels` observation block. Omitting it keeps a fresh run map-free and writes a v2 manifest whose task fingerprint is byte-identical to genuine pre-map runs; passing `--map classic`/`--map river`/`--map delta`/`--map lake` binds the task identity to `classic@1`/`river@1`/`delta@1`/`lake@1` and writes a v3 manifest that records `mapId`/`mapDefinitionVersion`. A resumed run always INHERITS the map identity from its manifest, so a genuine pre-map artifact still resumes and an explicit `--map` must match the manifest it resumes. Map identity is part of the task fingerprint, so a map-bound task is a distinct RL task from the map-free one.
