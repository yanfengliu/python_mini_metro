...
Policy lives in this section. ALL mechanics — current review model pins, exact CLI commands, sandbox flags, output extraction, and failure modes — live in the fleet-canonical runbook `../loop-ops/docs/skills/multi-cli-review.md` (review pins bump there, once for the whole fleet); `.claude/skills/multi-cli-review/SKILL.md` is this repo's thin stub for repo-specific notes, including the PowerShell-safe invocation forms. Read the canonical runbook, then the stub, before every multi-CLI session.
...
- Start from the baseline prompt in the fleet-canonical runbook (`../loop-ops/docs/skills/multi-cli-review.md`) and **enrich it with task-specific context** — task intent, files changed, validation performed, known baseline failures, and prior-review findings when iterating. The bare baseline returns generic feedback; useful reviews need the specifics. In this repo, also ask reviewers to flag process regressions, stale documentation, and missing validation.
- **Keep model IDs current.** Use the latest-family alias when a command is meant to track the newest model (for example, `opus[1m]`); bump pinned strings whenever a more capable fixed variant ships. The fleet-canonical runbook `../loop-ops/docs/skills/multi-cli-review.md` is the single bump site for review pins — bump there once for the whole fleet; this repo's stub `.claude/skills/multi-cli-review/SKILL.md` names any local pin sites to re-sync. Verify with a one-line smoke test (`echo "ok" | <cli> ...`) before committing the bump — silent fallback to an older model is the failure mode to guard against.
...
- The exact reviewer commands live in the fleet-canonical runbook `../loop-ops/docs/skills/multi-cli-review.md`; the PowerShell-safe prompt handling this repo needs lives in the repo stub `.claude/skills/multi-cli-review/SKILL.md`.
