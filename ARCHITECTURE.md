...
- `src/path_lifecycle.py` owns path creation, topology completion without automatic locomotive allocation, replacement, invalidation, selection, removal, color release, and button reassignment as a dependency-light stateless component. `src/fleet_management.py` separately owns stateless explicit assignment, reverse-order empty-locomotive eligibility, queued return, transactional detachment, and post-tick settlement behind public `Mediator` facades; it requires the shared canonical path-geometry validation before fleet effects and retains no host, path, collection, entity, factory, or callback between calls. `src/path_replacement.py` performs replacement preflight, semantic metro binding, and commit effects; the domain-light `src/path_replacement_geometry.py` validates canonical rounded geometry and builds a fully isolated off-live candidate; and `src/path_replacement_snapshot.py` owns exact rollback capture and restoration. `Mediator` remains the canonical owner of directly writable topology and fleet collections, maps, flags, factories, and entities.
- `src/passenger_flow.py` owns 17 passenger spawning, tick coordination, stop/exchange, delivery, waiting/game-over, scoped replanning, and bulk proposal-application operations as a dependency-light stateless component. Each call receives the current structural `PassengerFlowHost`; `Mediator` retains the exact public method signatures and canonical collections, RNG, clocks, progression, router, factories, and public hooks, while resolver thunks preserve each original lookup and mutation point without retaining a facade, callback, graph, entity, or cache between calls.
- `src/input_coordinator.py` owns path-button UI, layout, compatibility-render, mouse/keyboard, pause/speed, structured-action, and transient route-edit coordination as a dependency-light stateless component; `src/fleet_input.py` owns strict path index/id fleet-action selection and fleet release dispatch through the same public facade methods. `src/ui/fleet_button.py` binds plus/minus controls only to stable path-button slots, resolves the live path at use time, and derives enabled/queued rendering without mutating state. `src/input_coordinator_host.py` holds only its structural facade typing contract. Assigned-button redraws remain immutable `src/path_redraw.py` values, while `src/path_handle_input.py` owns thin two-phase selection/gesture cleanup, `src/path_handles.py` owns the weak idle selection plus immutable strong active edit, and `src/path_handle_geometry.py` builds primitive-only collision-resolved descriptors shared by input and rendering. Valid redraw and handle operations clear interface state before dispatching the existing public atomic replacement hook. `Mediator` retains the canonical UI, renderer, progression, topology, fleet, clock, and input state; false-to-true game over clears active pointer/edit references at the existing passenger-flow facade boundary.
...
- `src/rendering/network_renderer.py` owns separate bounded antialiased caches for the live network and one immutable selected-line preview, including arbitrary-slot temporary insertion, while sharing centered-lane geometry and the halo/color rasterizer. The cache-free `src/rendering/path_handle_renderer.py` draws primitive leader, marker, hit-envelope, and non-erasing removal feedback; `src/rendering/game_renderer.py` places leaders below entities, markers above stations/metros and below controls, projects endpoint-removal feedback onto the selected production lane, and renders available locomotives as the third HUD line with a property-first compatibility fallback. The config-owned `(0, 0, 840, 200)` HUD exclusion keeps every route-handle descriptor and registered-profile action round trip outside all three lines. `src/rendering/interpolation.py` tracks render-only previous/current metro snapshots, validates exact live segment identity, and safely rebases only equal physical transitions after a route edit; ambiguous stale topology falls back to the live pose. Fonts and surfaces are renderer-owned and lazy so state-only and headless sessions do not require a display.
...
- `src/agent_play.py` writes v4 playthrough records with explicit per-step/final deliveries, line credits, reward contract, post-reset overdue threshold, and the exact explicit-locomotive-assignment contract; persisted v4 fleet actions are replay-safe and index-only. Its legacy return and `score`/`final_score` fields continue to mean line credits; separately named delivery-returning helpers expose the canonical objective. Schema-less/v1 and literal v2 records reconstruct historical threshold `1`, v3 records validate and apply their recorded threshold, and all v1-v3 create operations use the shared pre-tick legacy assignment adapter while v4 uses ordinary explicit transitions and rejects UUID-bearing `path_id` fleet actions before stepping.
- `src/recursive_contract.py` owns strict immutable v1/v2/v3/v4 scenario and recorded-input validation plus reward/threshold/fleet reconstruction. V1 reconstructs `line_credits_delta` and threshold `1`; v2 preserves the explicit `deliveries` contract and threshold `1`; v3 requires deliveries plus a positive non-boolean threshold; v4 additionally requires the exact explicit-locomotive-assignment contract and index-only persisted fleet actions. `src/recursive_playtest.py` preserves its public version/validation imports as compatibility re-exports, executes every ordered operation, and writes strict JSON inputs, transcript rows, authored findings, and the run result. Historical v1-v3 create operations pass through the shared environment adapter that recreates their former automatic assignment before the original tick; v4 uses ordinary explicit transitions. Scenario v1 emits checkpoint v1, v2/v3 emit checkpoint v2, and v4 emits checkpoint v3.
- `src/recursive_checkpoint.py` converts observations and latent simulation state into UUID-free canonical JSON; `src/recursive_checkpoint_schema.py` owns version validation and normalization so both modules remain below 500 lines. Checkpoint v3 adds exact fleet total/assigned/available/queued scalars plus queue booleans on structured Metros and the ordered global-plus-path motion union. Generation verifies that the caller observation's ordered Metro identities match the live global collection before comparing every overlapping executable prefix field; normalization deep-copies v1/v2 evidence, synthesizes false queue state and complete fleet counts, and rejects alias, type, count, path, motion, or prefix disagreement. Genuine v1/v2 generation retains exact historical bytes only for nonqueued state and rejects any queued global or path-only Metro. Checkpoints also cover reward identity, topology, passengers and travel plans, progression and unlocks, spawning counters, dwell state, and Python/NumPy RNG state.
...
- `test/test_recursive_playtest.py` covers strict scenario/input validation, one transcript row per operation, and recorded-input replay; `test/test_recursive_threshold_schema.py` pins immutable v1/v2/v3 threshold reconstruction and checkpoint mapping plus the explicit v4 fleet contract; `test/test_recursive_checkpoint.py` covers UUID-free checkpoint construction, schema normalization, alias agreement, reward-contract identity, fleet/queue state, and latent-state observability. `test/test_overdue_threshold.py`, `test/test_agent_play_threshold.py`, and the GM-06b replay/agent modules cover default runtime overload semantics, agent evidence migration, historical adapter behavior, and index-only v4 actions without enlarging the mediator characterization suites.
...
- `test/test_game_renderer.py` covers lazy resources, layer order, metro interpolation, cached button fonts, the three-value HUD, and prepared game-over controls.
