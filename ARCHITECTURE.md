...
- `src/path_lifecycle.py` owns the 15 path creation, completion, replacement, invalidation, selection, removal, color-release, and button-reassignment transition algorithms as a dependency-light stateless component. `src/path_replacement.py` performs replacement preflight, semantic metro binding, and commit effects; the domain-light `src/path_replacement_geometry.py` validates canonical rounded geometry and builds a fully isolated off-live candidate; and `src/path_replacement_snapshot.py` owns exact rollback capture and restoration. Each call receives a structural `PathLifecycleHost` only for that transition; `Mediator` remains the canonical owner of directly writable topology collections, maps, flags, factories, entities, and public compatibility methods, and none of the helpers retains a facade backreference or mutable state between calls.
