...
- Added a cross-platform, descriptor-authorized civ-engine setup and verification boundary with ownership-checked lock/transaction cleanup and exact manual crash recovery, immutable provenance captures, complete non-generated `HEAD` byte authentication, exclusive-copy no-clobber publication plus final-path reauthentication, a strict root install graph and silent npm policy, Node-distribution npm plus pin-local TypeScript execution without shell/PATH lookup, an explicit trusted bootstrap with shared post-start taint detection, parser-shared canary selection, a fixed post-start zero-argument full-suite test guard, child-lifetime cooperative leases with pre/post-verification ownership checks, fail-closed shadow/tamper handling, and full-history Ubuntu/Windows CI dogfooding.
